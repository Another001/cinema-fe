...
export default function MessageItem({ type, text, time, name, color, seen }: MessageProps) {
  ...
  return (
    <div className={`flex ${isOut ? 'justify-end' : 'justify-start'} msg-anim`}>
      {/* Thay đổi: Dùng w-fit để khung ôm sát nội dung dài nhất */}
      <div className={`max-w-[80%] w-fit px-3 pb-1.5 pt-3 rounded-2xl ${isOut ? 'msg-out bg-[#202b36] rounded-br-md' : 'msg-in bg-[#2b5278] rounded-bl-md'}`}>
