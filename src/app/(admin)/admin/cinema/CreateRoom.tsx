...
import { CreateCinemaReq, CreateRoomReq } from '@/src/types/Cinema';
import cinemaApi from '@/src/api/cinema';
import MyLoading from '@/src/components/Loading';


const CreateRoom = ({setOpenCreateRoom, setIsLoading, isLoading, nowCinema, selectedCinema} :{setOpenCreateRoom : React.Dispatch<React.SetStateAction<boolean>>, setIsLoading: React.Dispatch<React.SetStateAction<boolean>>, isLoading: boolean, nowCinema?: string, selectedCinema?: number}) => {
  const handleSubmit = async (formData: FormData) => {
    setIsLoading(true);
    const data = Object.fromEntries(formData.entries());
    const payload : CreateRoomReq = {
      name: data.name as string,
      roomStatusId: 1,
      roomTypeId: Number(data.roomTypeId) as number,
      cinemaId: selectedCinema || 0
    }
    console.log("payload", payload)
    try{
      await cinemaApi.createRoom(payload);
      alert("Tao phong chieu thanh cong");
      setOpenCreateRoom(false);
    }
    catch{
      alert("Tao phong chieu that bai")
    }
    finally{
      setIsLoading(false)
    }
