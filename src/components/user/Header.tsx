...
export default function Header() {
  ...
  const {user, logoutSuccess} = useAuthContext();
    ...
    <div className="sticky top-0 z-[100] flex items-center hero-bg text-white justify-between px-12 py-5 border-b border-white/5">
      ...
      <div className="hidden md:flex items-center gap-8 text-sm font-medium">
        {user ? (
          ...
          <div className="relative group py-2">
            ...
            <div className="absolute top-full right-0 mt-2 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-2 group-hover:translate-y-0 transition-all duration-300 z-[110]">
              <div className="bg-[#1a1a1a]/95 backdrop-blur-xl border border-yellow-500/20 rounded-2xl overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
                ...
                {/* Lựa chọn: Xem vé */}
                <Link href="/user/my-ticket" className="flex items-center gap-3 px-5 py-4 hover:bg-yellow-500/10 hover:text-yellow-500 transition-colors border-b border-white/5">
                  <Ticket size={18} />
          ...
          <Link href="/login" className="text-gray-300 hover:text-yellow-500 transition relative z-20">
            Đăng nhập
          </Link>
        )}
